...
func newDefaultFiller() *Filler {
	...
	//2017-9-5:在此项目,defaults库和toml配合使用,方便进行默认值配置,但两个库支持time.Duration的方式不一样,defaults库比较小,简单稳定,所以选择在这里修改代码,支持config.Duration
	types["gobase/config.Duration"] = func(field *FieldData) {
		d, _ := time.ParseDuration(field.TagValue)
